  ...
  for (const [periodCode, periodLabel] of periods) {
    ...
    for (let run = 1; run <= repetitions; run += 1) {
      ...
      rows.push({
        ...
        scaler: data.model_config?.scaler ?? "",
        transform: data.model_config?.transform ?? "",
        ...
        loyalty_active_date_count: data.loyalty?.source_counts?.active_date || 0,
        loyalty_transaction_span_count: data.loyalty?.source_counts?.transaction_span || 0,
        loyalty_unavailable_count: data.loyalty?.source_counts?.unavailable || 0,
        loyalty_minimum_days: data.loyalty?.minimum_days ?? "",
        loyalty_maximum_days: data.loyalty?.maximum_days ?? "",
        loyalty_average_days: data.loyalty?.average_days ?? "",
...
const summaries = periods.map(([periodCode, periodLabel]) => {
  ...
  return {
    ...
    loyalty_transaction_span_count: selected[0]?.loyalty_transaction_span_count || 0,
    loyalty_unavailable_count: selected[0]?.loyalty_unavailable_count || 0,
    loyalty_average_days: selected[0]?.loyalty_average_days || 0,
    ...stats(selected.map((row) => Number(row.duration_seconds))),
    optimal_k: selected[0]?.optimal_k ?? "",
    scaler: selected[0]?.scaler ?? "",
...
fs.writeFileSync(
  ...
  [
    ...
    `- Loyalty fallback tersedia untuk ${rows[0]?.loyalty_transaction_span_count || 0} pelanggan; ${rows[0]?.loyalty_unavailable_count || 0} pelanggan tanpa riwayat relevan bernilai nol. Rata-rata Loyalty ${Number(rows[0]?.loyalty_average_days || 0).toFixed(2)} hari.`,
